...
version = 99.0.0
provider_path = registry.terraform.io/styczynski/kubectl-query/$(version)/darwin_amd64/

install_macos:
	go build -o terraform-provider-kubectl-query_$(version)
	mkdir -p ~/.terraform.d/plugins/$(provider_path)
	cp terraform-provider-kubectl-query_$(version) ~/.terraform.d/plugins/$(provider_path)
